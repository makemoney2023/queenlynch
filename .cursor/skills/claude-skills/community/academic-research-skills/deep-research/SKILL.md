...
> **Routing discipline (v3.9.2):** see `ROUTING.md (repo root)` "Routing Discipline (v3.9.2)" + `shared/references/intent_clarification_protocol.md` for cross-skill routing rules. This skill assumes routing has already settled — ambiguous cross-phase materials should have been clarified upstream.
...
Routing into Mode B requires explicit user signal — `/ars-<mode>` slash command or `[direct-mode]` prefix. Ambiguous cross-phase input defaults to clarification per `ROUTING.md (repo root)` Routing Discipline + `shared/references/intent_clarification_protocol.md`.
